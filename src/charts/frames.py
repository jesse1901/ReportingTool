...
"""
    ...
    def frame_group_by_user(
        ...
        # DuckDB Notes:
        # 1. "User", "Account", "Partition", "Start", "End" are reserved keywords -> Quoted.
        # 2. ANY_VALUE(eff."Account") used because we only GROUP BY eff."User".
        # 3. Divisions made float-safe (86400.0).
        ...
        SELECT
            ...
            printf('%2.0f%%', 100 * SUM(eff.Elapsed * eff.NCPUS * eff.CPUEff) / NULLIF(SUM(eff.Elapsed * eff.NCPUS), 0)) AS CPUEff,
        ...
        if not df.empty and 'Lost_CPU_days' in df.columns:
